...
from common.rag.models.intent_classifier import IntentClassifier
from consultant.api.routes import get_query_handler
...
from common.user_profile.message_helper import load_user_context_text

# 企业端业务关键词 - 命中则直接判定为需要检索
ENTERPRISE_BUSINESS_KEYWORDS = [
    # 留学基础关键词（继承客户端）
    "留学", "出国", "申请", "offer", "GPA", "雅思", "托福", "GRE", "GMAT",
    "文书", "推荐信", "海外", "美国", "英国", "澳洲", "加拿大", "日本", "韩国",
    "新加坡", "欧洲", "硕士", "博士", "本科", "签证", "奖学金",
    # 企业内部业务关键词
    "企业", "内部", "内幕", "消息", "动态", "趋势", "行业", "市场", "竞争",
    "渠道", "资源", "合作", "院校", "大学", "名额", "配额", "指标", "数据",
    "客户", "用户", "学员", "顾问", "规划师", "案例", "成功率", "费用", "定价",
    "策略", "运营", "业绩", "收入", "利润", "成本", "签约", "退款", "转化",
    "内推", "推荐", "实习", "就业", "职业", "归国", "海归", "薪资", "工作",
    "政策", "流程", "审批", "权限", "报告", "统计", "分析", "排名", "对比",
    "香港", "港大", "港中文", "大湾区", "IANG", "联申", "混申",
]

# 企业端通用关键词（问候/闲聊）
ENTERPRISE_GENERAL_KEYWORDS = [
    "你好", "hello", "hi", "嗨", "谢谢", "再见", "拜拜", "晚安", "在吗",
]

# 企业端意图识别器实例（使用企业端prompt_manager和关键词）
_consultant_intent_classifier = IntentClassifier(
    prompt_manager=consultant_prompt_manager,
    study_abroad_keywords=ENTERPRISE_BUSINESS_KEYWORDS,
    general_keywords=ENTERPRISE_GENERAL_KEYWORDS,
)
...
async def ensure_empty_conversation(
    ...
    try:
        ...
        if existing:
            logger.info(f"[规划师端] 复用已有空对话: id={existing['id']}")
            return existing

        # 没有空对话，新建一个
        conv = mgr.create_conversation(user_id=user_id)
        logger.info(f"[规划师端] 新建空对话: id={conv['id']}")
        return conv
    except Exception as e:
        logger.error(f"[规划师端] 确保空对话失败: {e}")
        raise HTTPException(status_code=500, detail=f"确保空对话失败: {str(e)}")


@router.get("/api/conversations")
async def list_conversations(current_user: dict = Depends(_get_current_user_from_header)):
    """获取所有会话列表

    【修复历史问题】chat/AI 对话分离：
    - 旧实现不按 dialogue_type 过滤，导致 contact_chat 联系人对话混入 AI 对话列表。
    - 新实现仅返回 dialogue_type='ai_chat' 的对话（兼容历史 NULL 数据），
      contact_chat 类型由 /api/consultant/contact-conversations 端点独立提供。
    """
    try:
        user_id = current_user.get("user_id") if current_user else None
        ...
        logger.error(f"[规划师端] 获取会话列表失败: {e}")
...
async def conversation_query(
    ...
    """
    在指定会话中提问（带对话上下文），使用SSE流式返回（规划师端）
    ...
    async def generate_events():
        ...
        try:
            ...
            logger.info(f"[规划师端] 对话历史加载完成: conversation_id={conversation_id}, 消息数={len(history)}, token={history_tokens}")
            ...
            logger.info(f"[规划师端] 对话查询: conversation_id={conversation_id}, question={question[:50]}")
            ...
            if history and len(history) > 0:
                ...
                try:
                    ...
                    rewrite_messages = consultant_prompt_manager.build_messages(
                        "query_rewrite",
                    ...
                    if rewritten and len(rewritten) >= len(question):
                        logger.info(f"[规划师端] 查询改写成功: '{question}' → '{rewritten}'")
                    ...
                    logger.warning(f"[规划师端] 查询改写失败: {e}")
                    step_rewrite["status"] = "skip"
            ...
            intent_result = _consultant_intent_classifier.classify(question)
            needs_retrieval = intent_result.get("intent") == "study_abroad"
            intent_text = "需要检索（企业业务问题）" if needs_retrieval else "无需检索（通用问题）"
            ...
            if not needs_retrieval:
                logger.info(f"[规划师端] 无需检索，直接LLM回答: {question}")
                ...
                general_msgs = consultant_prompt_manager.build_messages("general_answer", question=original_question)
                ...
                while True:
                    try:
                        ...
                        if isinstance(chunk, tuple) and chunk[0] == "error":
                            logger.error(f"[规划师端] LLM流式生成错误: {chunk[1]}")
                            break
            ...
            if context is not None:
                ...
                logger.info(f"[规划师端] Redis检索结果命中: {question}")
            else:
                ...
                if db_context is not None:
                    ...
                    logger.info(f"[规划师端] 数据库查询命中，已缓存检索结果到Redis: {question}")
                    ...
                    logger.info(f"[规划师端] 数据库未命中，准备RAG检索: {question}")
                yield f"data: {json.dumps(step_bm25, ensure_ascii=False)}\n\n"
                await asyncio.sleep(0.005)
                ...
                if context is None:
                    ...
                    try:
                        ...
                        if rag_context:
                            ...
                            logger.info(f"[规划师端] RAG检索成功，已缓存到Redis: {question}")
                        else:
            ...
            logger.error(f"[规划师端] 对话查询异常: {e}", exc_info=True)
