"""QueryHandler - 用户查询处理器，提供Redis缓存、BM25匹配、数据库查询、RAG检索等底层能力"""
import redis
import psycopg2
...
class QueryHandler:
    ...
    def __init__(
        ...
        """
        初始化QueryHandler，建立Redis、BM25、数据库连接
        ...
        if redis_client is not None:
            ...
            logger.info("QueryHandler复用外部Redis连接")
        else:
            self._init_redis()

        if bm25_retriever is not None:
            self.bm25_retriever = bm25_retriever
            logger.info("QueryHandler复用外部BM25检索器")
        else:
            self._init_bm25()

        if db_available is not None:
            self.db_pool = db_available
            logger.info(f"QueryHandler复用外部数据库状态: {'可用' if db_available else '不可用'}")
        else:
            self._init_db_pool()
    ...
    def _init_redis(self):
        """初始化Redis连接"""
        try:
            if not ClientRedisConfig.validate():
                logger.warning("Redis配置不完整，Redis功能将不可用")
                return

            self.redis_client = redis.Redis(**ClientRedisConfig.get_connection_params())
    ...
    def _init_db_pool(self):
        """创建数据库连接池"""
        try:
            if not ClientDatabaseConfig.validate():
                logger.warning("数据库配置不完整")
                return

            if QueryHandler._db_pool_instance is None:
                with QueryHandler._pool_lock:
                    if QueryHandler._db_pool_instance is None:
                        conn_params = ClientDatabaseConfig.get_connection_params()
                        QueryHandler._db_pool_instance = psycopg2.pool.SimpleConnectionPool(
                            minconn=1,
                            maxconn=5,
                            **conn_params
                        )
            ...
            self.db_pool = QueryHandler._db_pool_instance is not None
            if self.db_pool:
    ...
    def redis_exact_match(self, question: str) -> Optional[str]:
        ...
        try:
            key = f"{ClientRedisConfig.KEY_PREFIX_RETRIEVAL}{question}"
            result = self.redis_client.get(key)

            if result:
                self.redis_client.expire(key, ClientRedisConfig.TTL)
    ...
    def query_database(self, question: str) -> Optional[str]:
        ...
        pool = QueryHandler._db_pool_instance
        if pool is None:
    ...
    def cache_retrieval(self, question: str, context: str) -> bool:
        ...
        try:
            key = f"{ClientRedisConfig.KEY_PREFIX_RETRIEVAL}{question}"
            self.redis_client.setex(key, ClientRedisConfig.TTL, context)
            logger.info(f"检索结果已缓存到Redis: {question} (TTL={ClientRedisConfig.TTL}s)")
            return True
        except Exception as e:
    ...
    def fallback_to_llm(self, question: str, history_messages: list = None) -> Optional[str]:
        ...
        try:
            ...
            # 构建系统提示
            system_messages = prompt_manager.build_messages("fallback_answer", question=question)
