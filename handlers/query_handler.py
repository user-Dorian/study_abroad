...
class QueryHandler:
    ...
    def _init_redis(self):
        """初始化Redis连接"""
        try:
            if not RedisConfig.validate():
                logger.warning("Redis配置不完整，Redis功能将不可用")
                return

            self.redis_client = redis.Redis(**RedisConfig.get_connection_params())
    ...
    def redis_exact_match(self, question: str) -> Optional[str]:
        ...
        try:
            ...
            if result:
                self.redis_client.expire(key, RedisConfig.TTL)
    ...
    def cache_retrieval(self, question: str, context: str) -> bool:
        ...
        try:
            key = f"{RedisConfig.KEY_PREFIX_RETRIEVAL}{question}"
            self.redis_client.setex(key, RedisConfig.TTL, context)
            logger.info(f"检索结果已缓存到Redis: {question} (TTL={RedisConfig.TTL}s)")
            return True
        except Exception as e:
