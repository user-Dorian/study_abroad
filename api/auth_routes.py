...
from conversation.repository import _ensure_utc_iso
...
def _get_connection():
    """获取数据库连接"""
    from conversation.repository import _get_connection
    return _get_connection()


def _release_connection(conn):
    """释放数据库连接"""
    from conversation.repository import _release_connection
...
async def _async_find_user_by_username(username: str) -> Optional[dict]:
    """根据用户名查找用户（异步版）"""
    try:
        row = await AsyncDatabasePool.execute_one(
            "SELECT id, username, password_hash, email, display_name, created_at "
...
async def _async_find_user_by_id(user_id: str) -> Optional[dict]:
    """根据用户ID查找用户（不返回密码哈希，异步版）"""
    try:
        row = await AsyncDatabasePool.execute_one(
...
async def _async_create_user(username: str, password_hash: str, email: Optional[str] = None) -> dict:
    """创建新用户（异步版）

    使用 asyncpg，唯一约束违反通过 asyncpg.UniqueViolationError 捕获，
    与同步版 psycopg2.errors.UniqueViolation 行为一致。
    """
    import asyncpg
    ...
    try:
        ...
        row = await AsyncDatabasePool.execute_one(sql, user_id, username, password_hash, email, now, now)
        return {
            "id": str(row["id"]),
            "username": row["username"],
            "email": row["email"],
...
async def _async_update_user(user_id: str, email: Optional[str] = None, display_name: Optional[str] = None) -> Optional[dict]:
    """更新用户信息（异步版）"""
    now = datetime.utcnow()
    try:
        sql = (
        ...
        row = await AsyncDatabasePool.execute_one(sql, email, display_name, now, user_id)
        if row is None:
            return None
        return {
            "id": str(row["id"]),
...
async def register(request: RegisterRequest):
    ...
    """
    try:
        # 检查用户名是否已存在（阶段2数据库异步化：调用异步函数，不再用 asyncio.to_thread 包装）
        existing = await _async_find_user_by_username(request.username)
        if existing is not None:
            raise HTTPException(status_code=409, detail="用户名已被注册")

        # 创建用户（阶段2数据库异步化：调用异步函数）
        password_hash = hash_password(request.password)
        user = await _async_create_user(request.username, password_hash, request.email)
...
async def login(request: LoginRequest):
    ...
    """
    try:
        # 查找用户（阶段2数据库异步化：调用异步函数，不再用 asyncio.to_thread 包装）
        user = await _async_find_user_by_username(request.username)
        if user is None:
...
async def get_me(current_user: dict = Depends(require_user)):
    ...
    """
    try:
        # 阶段2数据库异步化：调用异步函数，不再用 asyncio.to_thread 包装
        user = await _async_find_user_by_id(current_user["user_id"])
        if user is None:
...
async def update_me(request: UpdateUserRequest, current_user: dict = Depends(require_user)):
    ...
    try:
        # 阶段2数据库异步化：调用异步函数，不再用 asyncio.to_thread 包装
        user = await _async_update_user(current_user["user_id"], request.email, request.display_name)
        if user is None:
            raise HTTPException(status_code=404, detail="用户不存在")
