...
class BM25Retriever:
    ...
    def __init__(self, index_dir: Optional[str] = None):
        ...
        if index_dir is None:
            self.index_dir = Path(__file__).resolve().parent.parent.parent / "bm25_index"
        else:
    ...
    # ====== 阶段5异步化改造：异步版search（保留同步search不变） ======

    async def async_search(self, query: str, top_k: int = 5) -> list:
        """异步版BM25检索（通过asyncio.to_thread包装CPU密集操作）
