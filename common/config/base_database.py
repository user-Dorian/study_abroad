...
class DatabaseConfig:
    ...
    # 阶段2数据库异步化：asyncpg 异步连接池配置
    ASYNC_POOL_MIN_SIZE = int(os.getenv("ASYNC_DB_POOL_MIN_SIZE", "2"))
    ASYNC_POOL_MAX_SIZE = int(os.getenv("ASYNC_DB_POOL_MAX_SIZE", "20"))
    ...
    @classmethod
    def get_async_dsn(cls) -> str:
        """阶段2数据库异步化：获取 asyncpg 使用的 DSN

        返回 postgresql://user:password@host:port/database 格式，
        供 asyncpg.create_pool(dsn=...) 使用。
        """
        return (
