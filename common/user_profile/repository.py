...
from common.conversation.repository import (
    _ensure_utc_iso,
    _get_connection,
    _release_connection,
    _affected_count,
...
def _async_parse_worker(document_id: str, user_id: str):
    ...
    """
    import psycopg2
    from common.user_profile.document_parser import parse_document
...
# ====== 阶段2数据库异步化：异步用户资料/文档仓库类（保留同步类不变，新增 asyncpg 版本） ======


class AsyncUserProfileRepository:
    """异步用户资料仓库（asyncpg）

    与同步 UserProfileRepository 行为一致，SQL 占位符从 %s 改为 $1, $2, ...
    """
    ...
    async def get_profile(self, user_id: str) -> Optional[dict]:
        ...
        try:
            ...
            return {
                ...
                "industry": row["industry"],
                "experience_years": row["experience_years"],
                # skills 为 TEXT[]，asyncpg 原生返回 list
                "skills": row["skills"] or [],
                "bio": row["bio"],
                "created_at": _ensure_utc_iso(row["created_at"]),
                "updated_at": _ensure_utc_iso(row["updated_at"]),
    ...
    async def create_or_update_profile(
        ...
        Args:
            ...
            industry: 行业领域
            experience_years: 工作年限
            skills: 技能标签列表（TEXT[]，asyncpg 原生支持 list）
            bio: 个人简介
        ...
        try:
            ...
            row = await AsyncDatabasePool.execute_one(
                sql, user_id, nickname, occupation, industry,
...
class AsyncUserDocumentRepository:
    """异步用户文档仓库（asyncpg）

    与同步 UserDocumentRepository 行为一致，SQL 占位符从 %s 改为 $1, $2, ...
    file_content 为 BYTEA，asyncpg 直接传 bytes / 返回 bytes，无需 psycopg2.Binary 包装。
    """
    ...
    async def create_document(
        ...
        Args:
            ...
            file_type: 文件类型(PDF/DOCX/TXT)
            file_size: 文件大小(bytes)
            file_content: 原始文件二进制内容（BYTEA，asyncpg 直接传 bytes）
            parsed_text: 解析后的文本（同步解析传入，异步解析传 None）

        Returns:
            dict: 文档信息
        """
        try:
            # 如果有解析文本，状态为 completed；否则为 uploading（异步解析）
            if parsed_text:
                sql = (
                    "INSERT INTO user_documents "
                    "(user_id, filename, file_type, file_size, file_content, parsed_text, parse_status) "
                    "VALUES ($1, $2, $3, $4, $5, $6, 'completed') "
                    "RETURNING id, user_id, filename, file_type, file_size, file_content, "
                    "parsed_text, parse_status, upload_time"
                )
                row = await AsyncDatabasePool.execute_one(
                    sql, user_id, filename, file_type, file_size, file_content, parsed_text
                )
            else:
                sql = (
                    "INSERT INTO user_documents "
                    "(user_id, filename, file_type, file_size, file_content, parse_status) "
                    "VALUES ($1, $2, $3, $4, $5, 'uploading') "
                    "RETURNING id, user_id, filename, file_type, file_size, file_content, "
                    "parsed_text, parse_status, upload_time"
                )
                row = await AsyncDatabasePool.execute_one(
                    sql, user_id, filename, file_type, file_size, file_content
                )

            logger.info(
                f"文档已上传: user_id={user_id}, filename={filename}, "
    ...
    async def get_document_by_id(self, document_id: str, user_id: str) -> Optional[dict]:
        """根据ID获取文档信息
        ...
        try:
            ...
            row = await AsyncDatabasePool.execute_one(sql, document_id, user_id)
            if row is None:
                return None
            return {
                "id": str(row["id"]),
    ...
    async def update_parse_status(self, document_id: str, status: str):
        """更新文档解析状态
        ...
        try:
            await AsyncDatabasePool.execute_command(
                "UPDATE user_documents SET parse_status = $1 WHERE id = $2",
                status, document_id,
            )
            logger.debug(f"文档解析状态已更新: id={document_id}, status={status}")
        except Exception as e:
    ...
    async def update_parsed_text(self, document_id: str, parsed_text: str, status: str = "completed"):
        """更新文档解析文本和状态
        ...
        try:
            await AsyncDatabasePool.execute_command(
                "UPDATE user_documents SET parsed_text = $1, parse_status = $2 WHERE id = $3",
                parsed_text, status, document_id,
            )
            logger.info(
                f"文档解析完成: id={document_id}, status={status}, text_len={len(parsed_text)}"
            )
        except Exception as e:
            logger.error(f"更新解析文本失败: id={document_id}, error={e}")
            raise
    ...
    async def delete_document(self, document_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            deleted = _affected_count(status) > 0
            if deleted:
                logger.info(f"文档已删除: id={document_id}")
            return deleted
    ...
    async def get_user_parsed_texts(self, user_id: str) -> List[str]:
        """获取用户所有已解析完成的文档文本（用于拼接system message）
        ...
        Returns:
            list[str]: 解析文本列表（仅包含 parse_status='completed' 的文档）
        """
        try:
            sql = (
                "SELECT parsed_text FROM user_documents "
                "WHERE user_id = $1 AND parse_status = 'completed' AND parsed_text IS NOT NULL"
            )
            rows = await AsyncDatabasePool.execute_query(sql, user_id)
            return [row["parsed_text"] for row in rows if row["parsed_text"]]
