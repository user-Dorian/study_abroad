...
企业端（规划师端）使用此模块（依赖 common.config.base_settings / common.user_profile.repository / common.utils.logger）。
客户端对应 user_profile/message_helper.py，逻辑一致但依赖根目录包路径。
"""
...
def load_user_context_text(user_id: Optional[str]) -> Tuple[str, str]:
    ...
    Returns:
        ...
        - 简历每份截取前 BaseConfig.USER_DOC_MAX_CHARS_PER_DOC 字（避免token过长）
    ...
    try:
        ...
        # 2. 加载简历文档内容（仅取解析完成的）
        max_chars = BaseConfig.USER_DOC_MAX_CHARS_PER_DOC
        ...
        if user_profile_text or user_docs_text:
            logger.info(
                f"[规划师端] 用户上下文已加载: user_id={user_id}, "
                f"profile_len={len(user_profile_text)}, docs_len={len(user_docs_text)}"
            )
    except Exception as e:
        # 加载失败不影响主流程，仅记录警告
        logger.warning(f"[规划师端] 加载用户上下文失败（不影响主流程）: user_id={user_id}, error={e}")
        return ("", "")
